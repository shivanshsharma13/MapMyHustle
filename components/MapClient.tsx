...
export default function MapClient({
  ...
  useEffect(() => {
    ...
    if (lastRunPoints.length) {
      setBounds(pick(lastRunPoints));
    } else if (allPoints.length) {
      setBounds(pick(allPoints));
    } else {
      setBounds([[20.5937, 78.9629], [20.5937, 78.9629]]);
    }
  ...
  const downloadPortrait = async () => {
    ...
    try {
      ...
      a.download = `territories-portrait-${ts}.png`;
      a.click();
    } catch (e) {
  ...
  return (
    <div className="relative w-full h-[70vh]">
      ...
      <div ref={exportRef} className="w-full h-full relative">
        {mounted && bounds && (
          <RL.MapContainer key={mapKey} className="w-full h-full" bounds={bounds} scrollWheelZoom>
            <RL.TileLayer
            ...
            {hexPolygons.map((poly, i) => (
              <RL.Polygon
                key={`hx-${i}`}
                positions={poly}
                pathOptions={{ weight: 1, fillOpacity: 0.18, opacity: 0.35, color: "#f59e0b", fillColor: "#f59e0b" }}
              />
            ))}

            {/* Runs: green */}
            {runLines.map((pts, i) => (
              <RL.Polyline key={`run-${i}`} positions={pts} weight={3} opacity={0.9} pathOptions={{ color: "#22c55e" }} />
            ))}

            {/* Rides: blue */}
            {rideLines.map((pts, i) => (
              <RL.Polyline key={`ride-${i}`} positions={pts} weight={3} opacity={0.9} pathOptions={{ color: "#3b82f6" }} />
            ))}
          </RL.MapContainer>
        )}
      </div>
